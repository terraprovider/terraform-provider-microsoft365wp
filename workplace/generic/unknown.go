...
//
// Lifted from https://github.com/hashicorp/terraform-provider-awscc/blob/v0.48.0/internal/generic/unknown.go
//
...
func unknownValuePaths(ctx context.Context, path *tftypes.AttributePath, val tftypes.Value) ([]*tftypes.AttributePath, error) {
	if !val.IsKnown() {
...
func SetUnknownValuesFromResourceModel(ctx context.Context, state *tfsdk.State, unknowns []*tftypes.AttributePath, val tftypes.Value) error {
	...
	// Copy all unknown values from val to destination State.
	for _, path := range unknowns {
		path, diags := attributePath(ctx, path, state.Schema)
		if diags.HasError() {
