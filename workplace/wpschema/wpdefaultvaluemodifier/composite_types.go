...
func (attributePlanModifier *objectDefaultValueAttributePlanModifier) PlanModifyObject(ctx context.Context, request planmodifier.ObjectRequest, response *planmodifier.ObjectResponse) {
	defaultValuePlanModify(ctx, &response.Diagnostics, request.Config.Schema.(rsschema.Schema), request.Path, request.ConfigValue, attributePlanModifier.defaultValueRaw, &attributePlanModifier.cachedDefaultValueAttributePlanModifier, &response.PlanValue)
}
